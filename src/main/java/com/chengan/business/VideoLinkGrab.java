...
public class VideoLinkGrab {
	...
	public static void main(String[] args) {
		...
		videoLinkGrab.saveData("http://www.80s.tw/movie/list-p");
	}
		...
		try {
			...
			for (Map.Entry<String, String> mapping : videoLinkMap.entrySet()) {
				...
				PreparedStatement pStatement = connection
						.prepareStatement("insert into movie(Id,MovieName,MovieLink) values(?,?,?)");
				pStatement.setString(1, UUID.randomUUID() + "");
				pStatement.setString(2, mapping.getKey());
				pStatement.setString(3, mapping.getValue());
	...
	private Map<String, String> crawlLinks(String oldLinkHost, Map<String, Boolean> oldMap) {
		...
		for (Map.Entry<String, Boolean> mapping : oldMap.entrySet()) {
			...
			if (!mapping.getValue()) {
				...
				// ((Object) oldMap).replace(oldLink, false, true);
			}
		}
		// 有新链接，继续遍历
		if (!newMap.isEmpty()) {
	...
	public boolean checkUrl(String url) {
		Pattern pattern = Pattern.compile("http://www.80s.tw/movie/list-p\\d*");
