...
<!--       	<div class="col"><button id="buttondetail" data-toggle="modal" data-target="#myModal" class="btn yellow-btn" type="button">新增</button></div> -->
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="width:650px;margin: 0 auto;">
      ...
      <div class="user-details">
        ...
        <form class="form-horizontal"  id="form" action="${ctx}/template/sysmodule/saveOrUpdate" method="post"  style="width:600px;margin: 0 auto;">
          <p>
          	<input type="hidden" name="id"  id="id" value="">
          	<input type="hidden" name="modulePic"  id="modulePic" value="">
          </p>		  
		  <div class="form-group">
		    <label for="tagName" class="col-sm-2 control-label">模块名称</label>
		    <div class="col-sm-10">
		      <input type="text"  name="moduleName" class="form-control"  id="moduleName" placeholder="">
		    </div>
		  </div>
		  <div class="form-group">
		  ...
		  <div class="form-group">
		    ...
		    <div class="col-sm-10">
		      <input type="text"  name="moduleUrl" class="form-control"  id="moduleUrl" placeholder="">
		    </div>
		  </div>
		  <div class="form-group">
		    <label for="tagName" class="col-sm-2 control-label">模块描述</label>
		    <div class="col-sm-10">
		      <input type="text"  name="moduleDesc" class="form-control"  id="moduleDesc" placeholder="">
		    </div>
		  </div>
		  <div class="form-group">
		    <label for="tagName" class="col-sm-2 control-label">模块编号</label>
		    <div class="col-sm-10">
		      <input type="number"  name="moduleOrder" class="form-control"  id="moduleOrder"  value="" placeholder="">
		    </div>
		  </div>
		  <div class="form-group">
  ...
  $(function() {
	  ...
	  var storeUrl = ctx+'/template/sysmodule/getSysmoduleListPage';
	  var pageSize = 10;
	  var operateEvents = {
	      'click .editor-btn': function (e, value, row, index) {
	          $("#id").val(row.id);
	          $("#moduleName").val(row.moduleName);
	          $("#moduleDesc").val(row.moduleDesc);
	          $("#modulePic").val(row.modulePic);
	          $("#moduleUrl").val(row.moduleUrl);
	          $("#moduleOrder").val(row.moduleOrder);
	      ...
	      'click .del-btn': function (e, value, row, index) {
	    	  if(window.confirm("您确定要删除吗？")){
			   	  $.ajax({
			          type: "POST",
			          url: ctx+"/template/sysmodule/deleteById",
			          data: {"id":row.id},
			          dataType: "json",
			          success: function(obj){
			        	  window.location.href= ctx+'/template/login/toGrid?gridName=sysmoduleGrid';
			          }
			      }); 	    		  
	    	  }
	      }
