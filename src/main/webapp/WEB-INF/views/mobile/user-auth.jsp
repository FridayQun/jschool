  ...
  <div class="user-auth-box">
    <form class="modifyForm" action="#" method="post">
      ...
      <div class="form-group">
        ...
        <div class="form-input">
          <input class="input-style" type="text" name="school" value="" placeholder="请输入学校名称">
        </div>
      </div>
      <div class="form-group">
        <div class="form-label">学院：</div>
        <div class="form-input">
          <input class="input-style" type="text" name="branch" value="" placeholder="请输入学院名称">
        </div>
      </div>
      <div class="form-group">
        <div class="form-label">社团：</div>
        <div class="form-input">
          <input class="input-style" type="text" name="club" value="" placeholder="请输入您所在的社团">
        </div>
      </div>
      <div class="form-group">
        <div class="form-label">专业：</div>
        <div class="form-input">
          <input class="input-style" type="text" name="major" value="" placeholder="请输入专业名称">
        </div>
      </div>
      <div class="form-group">
        <div class="form-label">入学时间：</div>
        <div class="form-input">
          <input class="input-style" type="date" name="eduStarttime" value="2017-09-01">
        </div>
      </div>
      ...
      <p><small>学生用户请上传学生证、校园卡等证件</small></p>
    ...
    $(function() {
    	//提交
    	 $('.modifyForm').on('submit',function(event){
        	...
        	var eduStarttime = $(this).find('input[name="eduStarttime"]').val();
        	...
        	if(!realname){
        		...
        		JKApp.dialog({message:"请输入学校名称!"});
        	}else if(!eduStarttime){
        		JKApp.dialog({message:"请输入学时间!"});
        	}else if(document === undefined || document === '' || document === null){
        		JKApp.dialog({message:"请上传身份核实照片!"});
        	}else{
        		formData.push({
    		 		name:"uid",
    		 		value:$('#userid').val()
