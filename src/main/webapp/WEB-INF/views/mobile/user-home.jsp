	...
	function render(data,empty) {
		if(data) {
			var nodeList = data.map(function(item,index) {
				console.log(item);
				return(
					format.call('<li style="height: 100px;" class="horizontal-parting-line"><a href="{projectLink}"><div class="item-photo" style="width: 30%;float: left;height: 100%;"><img src="{imageUrl}" alt="" style="width: 100%;height: 100%;"></div><div class="item-desc" style="float: left;height: 100%;width: 70%; box-sizing: border-box; padding-left: 10px;"><p style="font-size: 15px;font-weight: bold;"><span style="    width: 70%;    display: inline-block;    overflow: hidden;    text-overflow: ellipsis;">{projectName}</span><span style="color:blue;float: right;">￥{moneyNeeds}</span>&nbsp;</p><p style="font-size: 8px;color: #b7b7b7;"><span style="margin-right: 10px;">{username}</span><i class="icon eye"></i><span style="margin-right: 5px;">{view}人浏览/{apply}人申请</span><i class="icon clock"></i><span style="">{createDate}</span>&nbsp;</p><p><small>{location}</small>&nbsp;</p><p><small><i class="icon clock"></i>{left}天后申请截止</small><span style="display:{selfSell} color: white;background: orange;float: right;padding: 0 10px;border-radius: 2px;">自营</span>&nbsp;</p></div></a></li>', {
