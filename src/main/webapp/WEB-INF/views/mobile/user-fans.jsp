...
<body>
  ...
  <script>
    ...
    $(function(){
      ...
      $.ajax({
          type: 'post',
          url: ctx+"/fansfollows/getfanslistpage",
          data: {pageSize:pageSize,pageNumber:pageNumber},
          aysnc:true,
          dataType: "json",
          success: function(data){
        	  if(data.fanslist.length>0){
        		  renderList(data.fanslist);
        		  $('.no-msg').remove();
        	  }else{
        		  $('body').append('<h3 class="no-msg" style="text-align: center;padding:2em 0;">您还没有任何粉丝！</h3>');
        		  autoShow();
        	  }
          }
      });
